<table border="1px" class="table table-striped table-bordered table-hover" style="padding: 10px;">
    <tr>
        ...
        <td class="table-header">Last Login At:</td>
    ...
    <g:each in="${userList}" var="user">
        <tr>
            ...
            <g:if test="${user.isActive}">
                <td style="background-color: ${user.isActive == false ? '' : 'lightgreen'}">
                <g:link class="anchor-nounderline" controller="user" action="deactivate"
                        params="[userId: user.id]">Active</td></g:link>
            </g:if>
            <g:else>
                <td style="background-color: ${user.isActive == false ? '' : 'lightgreen'}">
                <g:link class="anchor-nounderline" controller="user" action="activate"
